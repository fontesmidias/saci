...
Aider, etc.). Cada PERFIL vira um "modelo":
    ...
    saci-code   -> perfil code   (gerar/refatorar codigo)
    saci-plan   -> perfil plan   (arquitetura, decisoes)
    saci-agent  -> perfil agent  (Cline/Continue: prompts grandes)
    saci-fast   -> perfil fast   (perguntas rapidas)
    saci-long   -> perfil long   (contexto grande)
    saci-pt     -> perfil pt     (portugues corporativo)
...
MODEL_PREFIX = "saci-"
LEGACY_PREFIX = "router-"   # nome antigo, ainda aceito
...
def resolve_profile(model: str) -> str:
    ...
    name = model
    for pre in (MODEL_PREFIX, LEGACY_PREFIX):
